...
{
    public sealed class Screen : BaseInPlaceFilter2
    {
        ...
        public Screen(Bitmap overlayImage) : base(overlayImage)
        {
            InitFormatTranslations();
        }
        ...
        public Screen(UnmanagedImage unmanagedOverlayImage) : base(unmanagedOverlayImage)
        {
            InitFormatTranslations();
        }
        ...
        protected override unsafe void ProcessFilter(UnmanagedImage image, UnmanagedImage overlay)
        {
            ...
            if (pixelFormat == PixelFormat.Format8bppIndexed || pixelFormat == PixelFormat.Format24bppRgb || (pixelFormat == PixelFormat.Format32bppRgb || pixelFormat == PixelFormat.Format32bppArgb))
            {
                ...
                for (int index = 0; index < height; ++index)
                {
                    ...
                    while (num5 < num2)
                    {
                        num6 = ~(byte)(((byte)~*numPtr1 * Factor) * ((byte)~*numPtr2 * Factor) * 255);
            ...
            else
            {
                ...
                for (int index = 0; index < height; ++index)
                {
                    ...
                    while (num3 < num2)
                    {
                        num4 = ~(byte)(((byte)~*numPtr3 * Factor) * ((byte)~*numPtr4 * Factor) * 255);
